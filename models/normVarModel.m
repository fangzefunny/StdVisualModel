classdef normVarModel < contrastModel 
    ...
    methods
        ...
        % init the model
        function model = normVarModel( param_bound, param_pbound, fittime )
            
            model = model@contrastModel();
           
            if (nargin < 3), fittime = 5; end
            if (nargin < 2), param_pbound = [ .1, 20; 1,  10;  .1, .5 ]; end
            if (nargin < 1), param_bound  = [ 0, 30; 0, 100; 0,   1  ]; end
            
            para_mnum = 3;
            
            if size(param_bound,1) ~= para_mnum 
                error('Wrong Bound')
            elseif size(param_pbound, 1) ~= para_mnum 
    ...
    methods ( Static = true )
        ...
        function [losses, params, Rsquare, model] = fit( model, E_ori, BOLD_target, verbose, cross_valid )
            ...
            % call subclass
            [losses, params, Rsquare, model] = fit@contrastModel( model, E_ori, BOLD_target, verbose, cross_valid );
